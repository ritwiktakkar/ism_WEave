...
function createAdminChannel() {
  ...
  # now run the script that creates a channel. This script uses configtxgen once
  # to create the channel creation transaction and the anchor peer updates.
  scripts/createAdminChannel.sh "admin-channel" $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true
}

function createProductionChannel() {
  # Bring up the network if it is not already up.
  bringUpNetwork="false"
  ...
  # now run the script that creates a channel. This script uses configtxgen once
  # to create the channel creation transaction and the anchor peer updates.
  scripts/createProductionChannel.sh "production-channel" $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true
}
...
function networkDown() {
  ...
  COMPOSE_ORG6_BASE_FILES="-f addOrg6/compose/${COMPOSE_FILE_ORG6_BASE} -f addOrg6/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG6_BASE}"
  COMPOSE_ORG6_COUCH_FILES="-f addOrg6/compose/${COMPOSE_FILE_ORG6_COUCH} -f addOrg6/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG6_COUCH}"
  COMPOSE_ORG6_CA_FILES="-f addOrg6/compose/${COMPOSE_FILE_ORG6_CA} -f addOrg6/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG6_CA}"
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG6_FILES} down --volumes --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} ${COMPOSE_ORG6_FILES} down --volumes
  else
    fatalln "Container CLI  ${CONTAINER_CLI} not supported"
  fi
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addOrg6/fabric-ca/org6/msp addOrg6/fabric-ca/org6/tls-cert.pem addOrg6/fabric-ca/org6/ca-cert.pem addOrg6/fabric-ca/org6/IssuerPublicKey addOrg6/fabric-ca/org6/IssuerRevocationPublicKey addOrg6/fabric-ca/org6/fabric-ca-server.db'
    # remove channel and script artifacts
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf *channel-artifacts log.txt *.tar.gz && rm -rf packaged*'
  fi
}
...
COMPOSE_FILE_ORG6_BASE=compose-org6.yaml
# use this as the docker compose couch file for org6
COMPOSE_FILE_ORG6_COUCH=compose-couch-org6.yaml
# certificate authorities compose file
COMPOSE_FILE_ORG6_CA=compose-ca-org6.yaml
...
if [[ $# -ge 1 ]] ; then
  ...
  if [[ "$key" == "createChannel" ]]; then
      ...
      shift
  # check for the createAdminChannel subcommand
  elif [[ "$key" == "createAdminChannel" ]]; then
      export MODE="createAdminChannel"
      shift
  # check for the createProductionChannel subcommand
  elif [[ "$key" == "createProductionChannel" ]]; then
      export MODE="createProductionChannel"
      shift
...
if [ "$MODE" == "prereq" ]; then
  ...
  createChannel $BFT
elif [ "$MODE" == "createAdminChannel" ]; then
  infoln "Creating channel admin-channel."
  infoln "If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE} ${CRYPTO_MODE}"
  createAdminChannel $BFT
elif [ "$MODE" == "createProductionChannel" ]; then
  infoln "Creating channel production-channel."
  infoln "If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE} ${CRYPTO_MODE}"
  createProductionChannel $BFT
elif [ "$MODE" == "down" ]; then
