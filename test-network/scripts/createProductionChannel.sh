...
if [ ! -d "production-channel-artifacts" ]; then
	mkdir production-channel-artifacts
fi
...
createChannelGenesisBlock() {
  setGlobals 1
	...
	if [ $bft_true -eq 1 ]; then
		configtxgen -profile ProductionChannel -outputBlock ./production-channel-artifacts/${CHANNEL_NAME}.block -channelID $CHANNEL_NAME
	else
		configtxgen -profile ProductionChannel -outputBlock ./production-channel-artifacts/${CHANNEL_NAME}.block -channelID $CHANNEL_NAME
	fi
	res=$?
...
FABRIC_CFG_PATH=$PWD/../config/
BLOCKFILE="./production-channel-artifacts/${CHANNEL_NAME}.block"
...
setAnchorPeer 3
infoln "Settingx anchor peer for org4..."
setAnchorPeer 4
