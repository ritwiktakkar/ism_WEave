...
if [ $channel_name=="admin-channel" ]; then
    osnadmin channel join --channelID ${channel_name} --config-block ./admin-channel-artifacts/${channel_name}.block -o localhost:7059 --ca-file "$ORDERER_CA" --client-cert "$ORDERER_ADMIN_TLS_SIGN_CERT" --client-key "$ORDERER_ADMIN_TLS_PRIVATE_KEY" >> log.txt 2>&1
elif [ $channel_name=="production-channel" ]; then
    osnadmin channel join --channelID ${channel_name} --config-block ./production-channel-artifacts/${channel_name}.block -o localhost:7059 --ca-file "$ORDERER_CA" --client-cert "$ORDERER_ADMIN_TLS_SIGN_CERT" --client-key "$ORDERER_ADMIN_TLS_PRIVATE_KEY" >> log.txt 2>&1
else
