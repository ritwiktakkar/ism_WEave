...
if [ ! -d "admin-channel-artifacts" ]; then
	mkdir admin-channel-artifacts
fi
...
createChannelGenesisBlock() {
  setGlobals 1
	...
	if [ $bft_true -eq 1 ]; then
		configtxgen -profile AdminChannel -outputBlock ./admin-channel-artifacts/${CHANNEL_NAME}.block -channelID $CHANNEL_NAME
	else
		configtxgen -profile AdminChannel -outputBlock ./admin-channel-artifacts/${CHANNEL_NAME}.block -channelID $CHANNEL_NAME
	fi
	res=$?
...
FABRIC_CFG_PATH=$PWD/../config/
BLOCKFILE="./admin-channel-artifacts/${CHANNEL_NAME}.block"
